...
| HUG-AUD-018 | P3 | Architecture | DONE | Split oversized files by responsibility (`WorkspaceStore`, `EditorView`, `MarkdownStyler`). | Core modules are smaller with focused responsibilities. |
| HUG-AUD-019 | P2 | Testing | DONE | Add regression tests for all audit fixes (security, race, loading, parser). | New tests fail before fixes and pass after fixes. |
...
| ID | Pri | Area | Status | Task | Done When |
|---|---|---|---|---|---|
| HUG-AUD-028 | P2 | Feature | DONE | Add explicit section picker for new post creation. | User can choose target section before creating post. |
...
| HUG-AUD-035 | P3 | Security UX | DONE | Add explicit UI warnings for blocked path escapes/config anomalies. | Users get clear warnings when unsafe paths are rejected. |
